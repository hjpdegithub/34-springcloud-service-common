...
public class ExaminationPaperController {
    ...
    public ApiResult addOrUpdate(@RequestBody ExaminationAddOrUpdateDto dto){
        ...
        //计算抽题数量
        Integer j = dto.getJudgeRule().getSubjectNum()*dto.getJudgeRule().getFraction();
        Integer s = dto.getSingleRule().getSubjectNum()*dto.getSingleRule().getFraction();
        Integer m = dto.getMultipleRule().getSubjectNum()*dto.getMultipleRule().getFraction();
