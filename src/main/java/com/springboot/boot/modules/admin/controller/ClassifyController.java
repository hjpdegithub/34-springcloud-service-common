 ...
 **/
@RestController
@RequestMapping("/mp/classify")
@Slf4j
@Api(tags = "1.1", description = "分类管理【周京昊】")
@CrossOrigin
public class ClassifyController {

    @Autowired
    private ClassifyService classifyService;
    @ApiOperation(value = "分类的新增以及修改", notes="该接口根据一级分类主键id判断新增不传，修改需要传入")
    @PostMapping(value="/addOrUpdate")
    public ApiResult addOrUpdate( @RequestBody ClassifyAddOrUpdateDto dto){
        log.info("分类的新增以及修改----------参数对象输出{}", JSONObject.toJSON(dto));
        //判断一级分类名称是否相同
        ApiResult result = classifyService.addOrUpdate(dto);
        return result;
    }
